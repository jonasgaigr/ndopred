...
server <- function(input, output, session) {
  ...
  assessment_results <- eventReactive(input$run_calc, {
    ...
    withProgress(message = 'Accessing NDOP...', value = 0, {
      ...
      # This fixes the "object 'ROK' not found" error for ALL functions
      occ_all <- occ_raw
      ...
      # Note: calculate_pop_metrics handles renaming NAZ_LOKAL internally,
      # but we pass the dataframe with ROK already added.
  ...
  output$metrics_table <- renderTable({
    ...
    data.frame(
      "Metric" = c("Taxon", "Expert Category", "Expert Criteria", "EOO", "AOO", "Locs"),
