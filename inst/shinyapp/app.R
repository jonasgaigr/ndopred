...
server <- function(input, output, session) {
  ...
  raw_data <- eventReactive(input$run_calc, {
    ...
    withProgress(message = 'Accessing NDOP...', value = 0, {
      ...
      list(eoo=eoo, aoo=aoo, locs=locs, trend=trend, pop=pop_metrics, occ_all=occ_all, taxon_group=taxon_group, species=input$species_name)
    })
  })
  ...
  observeEvent(raw_data(), {
    req(raw_data())
    updateCheckboxInput(session, "use_pop", value = (raw_data()$taxon_group %in% get_pop_groups()))
  })
  ...
  assessment_results <- reactive({
    data <- raw_data()
    if (is.null(data)) {
      return(list(res = data.frame(Species=input$species_name, Category="DD", Criteria="", Trend_Perc=NA),
                  pop = list(total_mature=NA, decline_rate=NA, max_subpop=NA, fluct_ratio=NA)))
    }
    ...
    if (res$Category %in% c("LC", "DD")) {
      is_nt <- FALSE
      if (!is.na(res$Trend_Perc) && abs(as.numeric(res$Trend_Perc)) >= 20 && abs(as.numeric(res$Trend_Perc)) < 30) is_nt <- TRUE
      if (res$EOO_km2 > 0 && res$EOO_km2 < 30000 && res$Locations <= 15) is_nt <- TRUE
      if (input$use_pop && !is.na(pop$total_mature) && pop$total_mature >= 1000 && pop$total_mature < 2000) is_nt <- TRUE
      if (is_nt) { res$Category <- "NT"; res$Criteria <- "Close to criteria" }
    }

    res$Criteria <- gsub("^;|;$", "", gsub(";;", ";", new_crit))
    if(res$Criteria == "") res$Criteria <- "None"

    list(res=res, trend=data$trend, occ_all=data$occ_all, pop=pop)
  })
  ...
  output$verification_panel <- renderUI({
    ...
    tagList(
      ...
      fluidRow(
        ...
        column(4, wellPanel(
          h4("E. Quantitative"),
          selectInput("check_e", "Extinction Prob:", choices = c("None", "CR", "EN", "VU"))
        ))
      )
    )
  })

  # --- 5. EXPERT FINAL VERDICT ---
  expert_final <- reactive({
    ...
    if (length(input$check_a_type) > 0) {
      ...
      if (a_cat != "LC") { triggered_cats <- c(triggered_cats, a_cat); expert_codes <- c(expert_codes, paste0(input$check_a_type[1], paste(input$a_basis, collapse=""))) }
    }
    # B
  ...
  output$metrics_table <- renderTable({
    ...
    data.frame("Metric" = c("Taxon", "Expert Category", "Expert Criteria", "EOO", "AOO", "Locs", "Mature Indivs", "Fluctuation"),
               "Value" = c(res$Species, exp$category, exp$criteria, base::format(res$EOO_km2, nsmall=1),
                           base::format(res$AOO_km2, nsmall=1), as.character(res$Locations), mat_val, if(!is.na(pop$fluct_ratio)) paste0(pop$fluct_ratio, "x") else "-"))
  })
